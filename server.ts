...
Aqui está a letra com cifras da música:\n\n${content}\n\nPreencha a análise didática contendo o tom detectado de forma precisa, a escala correspondente, o campo harmônico diatônico de 7 acordes do tom, um mapeamento detalhado dos acordes individuais presentes na canção e seus respectivos graus funcionais (ex: I, IV, V7, vi, etc.), cadências melódicas ou funcionais encontradas (como II-V-I, ou progressões plagais como IV-IVm-I). Identifique e descreva acordes especiais como empréstimos modais (AEM), dominantes secundárias ou SubV7. IMPORTANTE: Se um acorde especial (como Fm em uma música no tom de C) não estiver na cifra original fornecida, inclua-o expressamente como uma "Dica de Arranjo Harmônico / Rearmonização sugerida para embelezamento", explicando como a banda pode inseri-lo para enriquecer o arranjo. Dê dicas práticas detalhadas direcionadas para tecladistas, violonistas/guitarristas, baixistas e ministros vocais executarem essa harmonia com total união e reverência no altar.`;
  ...
  // Helper function to guarantee Intro is ALWAYS item #1 in the audit result
  function ensureIntroFirst(divergences: any[], content: string, foundChordsInText: string[]) {
    const list = Array.isArray(divergences) ? [...divergences] : [];
    
    // Check if any divergence item already mentions intro or introdução
    ...
    if (introIndex > 0) {
      // Move existing intro item to position 0
      const [introItem] = list.splice(introIndex, 1);
      list.unshift(introItem);
    } else if (introIndex === -1) {
      // Create and prepend an Intro section if none was returned
      ...
      list.unshift({
        ...
        note: 'Análise da Introdução (Intro): Dedilhado do violão e fraseado de teclado/piano sustentando a harmonia de abertura no tom oficial do YouTube.'
      });
    }

    // Format section labels sequentially: "1. Introdução (Intro)", "2. Verso 1", etc.
    return list.map((item, idx) => {
      let rawName = (item.section || `Seção ${idx + 1}`).replace(/^[0-9]+\.\s*/, '');
      return {
        ...item,
  ...
  // POST endpoint to perform real YouTube Audio Chord Audit against song content
  app.post("/api/songs/audit-youtube-chords", async (req, res) => {
    const { title, artist, content, baseKey, youtubeUrl } = req.body;
    try {
      if (!title || !content) {
        return res.status(400).json({ error: "O título e o conteúdo da música são obrigatórios." });
      }

      // Extract actual unique chord tokens from provided content
      ...
      const systemInstruction = `Você é um maestro e produtor musical especialista em harmonia funcional e arranjos de louvor/worship contemporâneo. Sua missão é realizar a auditoria harmônica rigorosa de uma cifra fornecida pelo usuário, comparando-a EXATAMENTE com a gravação de áudio do YouTube da mesma canção.

REGRAS INEGOCIÁVEIS:
1. A PRIMEIRA SEÇÃO DA ANÁLISE DEVE SER A INTRODUÇÃO (INTRO): O primeiro item da lista de divergências DEVE SER OBRIGATORIAMENTE a "Introdução (Intro)", auditando o dedilhado do violão/teclado no início do áudio do YouTube vs a cifra enviada pelo usuário.
2. ORDEM CRONOLÓGICA DAS SEÇÕES: 1º Introdução (Intro), 2º Verso 1 / Primeira Parte, 3º Pré-Refrão, 4º Refrão, 5º Ponte / Solo.
3. FIDELIDADE À CIFRA DO USUÁRIO: No campo "orig", cite EXATAMENTE os acordes que o usuário tem escritos na sua cifra naquela seção.
4. AUDITORIA REFINADA: Identifique baixos invertidos (C/G, D/F#, G/B), notas pedal e extensões (C9, Em7, D4, G/B) do áudio oficial do YouTube.

Responda estritamente em formato JSON.`;

      const prompt = `AUDITORIA DE CIFRA X ÁUDIO DO YOUTUBE:
Música: "${title}"
Artista: "${artist || 'Desconhecido'}"
Tom sugerido: "${baseKey || 'C'}"
URL YouTube: "${youtubeUrl || 'Gravação Oficial'}"

CIFRA ATUAL FORNECIDA PELO USUÁRIO (LEIA COM TOTAL FIDELIDADE AO TEXTO):
---
${content}
---

INSTRUÇÕES OBRIGATÓRIAS:
1. COMECE OBRIGATORIAMENTE PELA INTRODUÇÃO (INTRO): A divergência nº 1 DEVE ser a "Introdução (Intro)".
2. LEIA OS ACORDES DA CIFRA DO USUÁRIO COM ABSOLUTA PRECISÃO:
   Os acordes detectados no texto fornecido incluem: ${foundChordsInText.join(', ')}.
   Ao preencher o campo "orig" das divergências, cite EXATAMENTE os acordes correspondentes da cifra do usuário acima.
3. AUDITE CONTRA O ÁUDIO REAL DO YOUTUBE:
   - Identifique como violão, teclado e baixo elétrico executam a abertura na Introdução e demais partes do áudio oficial do YouTube.
   - Verifique inversões (C/G, D/F#, G/B, D9/G, F/A) e extensões (C9, Em7, D4).
...
Retorne o JSON conforme o schema.`;
                ...
                properties: {
                  ...
                  divergences: {
                    ...
                    items: {
                      type: Type.OBJECT,
                      properties: {
                        section: { type: Type.STRING, description: "Section name, starting strictly with 'Introdução (Intro)', then 'Verso 1', 'Refrão', 'Ponte'" },
                        orig: { type: Type.STRING, description: "EXACT chords from the user's current chord sheet in this section" },
                        audio: { type: Type.STRING, description: "Real chords executed in the YouTube audio recording" },
                        note: { type: Type.STRING, description: "A detailed explanation of why the audio differs (e.g. bass inversion C/G, pedal note D9/G, 9th extension C9)" }
                      },
      ...
      if (responseText) {
        ...
        parsed.divergences = ensureIntroFirst(parsed.divergences || [], content, foundChordsInText as string[]);
        return res.json(parsed);
      }
      throw lastError || new Error("Failed to generate audit");
    } catch (error: any) {
      console.log("[Audit API] Local dynamic fallback applied");
      // Smart local dynamic fallback parsing the user's actual text and chords
      const chordRegex = /\b[a-gA-G][#b]?(?:m|maj|min|dim|aug|sus|add|alt|omit|no|M|[0-9]|\(|\)|\+|\-|\#|b|Δ|ø|°|º|ª)*(?:\/(?:[a-gA-G][#b]?|[0-9]+))?\b/gi;
      const foundChordsInText = Array.from(new Set((content.match(chordRegex) || []))) as string[];
      ...
      const fallbackDivs = [
        {
          ...
          note: `Análise da Introdução (Intro): A cifra atual possui [${foundChordsInText.slice(0, 2).join(', ')}]. No áudio do YouTube, o dedilhado do violão e o teclado fazem a introdução mantendo a nota Sol no pedal (C/G ou C9).`
        },
        {
          section: "Verso 1 / Primeira Parte",
          orig: sampleChords,
          audio: sampleChords.replace(/\bC\b/g, 'C9').replace(/\bD\b/g, 'D/F#').replace(/\bG\b/g, 'G/B'),
          note: `Acordes lidos diretamente da sua cifra: [${foundChordsInText.join(', ')}]. A gravação no YouTube mantém a condução com baixos invertidos e extensões de nona.`
        },
        {
          ...
          note: "Na gravação oficial do YouTube, o baixo elétrico e o piano sustentam a nota Sol com extensão de nona para manter o clima de adoração do refrão."
        }
      ];
      ...
      res.json({
        ...
        divergences: ensureIntroFirst(fallbackDivs, content, foundChordsInText as string[]),
        suggestedChords: content
          .replace(/\bC\b/g, 'C9')
          .replace(/\bD\b/g, 'D/F#')
          .replace(/\bEm\b/g, 'Em7')
      });
  ...
  async function scrapeCifraClub(url: string): Promise<{
    title: string;
    artist: string;
    key: string;
    bpm: number;
    ...
    // 1. Fetch HTML with custom headers to prevent blocking
    ...
    if (!responseHtml.ok) {
      throw new Error(`O site Cifra Club retornou o status HTTP ${responseHtml.status}. Verifique se o link está correto.`);
    }
    ...
    // 2. Extract Title and Artist from <title> tag
    const titleMatch = html.match(/<title>([\s\S]*?)<\/title>/i);
    let extractedTitle = "Música Importada";
    ...
    if (titleMatch) {
      ...
      if (parts.length >= 2) {
        ...
        // Remove suffix like "Cifra Club" if present in artist
    ...
    // Normalize key format under Liloupro regulations
    key = key
      .replace("m7m", "m7")
      .replace("min7", "m7")
      .replace("7+", "7M")
    ...
    // Check for high-res artist picture paths (e.g., in Cifra Club's CDN) first
    ...
    // Fallback to standard og:image if no specific artist CDN URL found
    if (!artistImageUrl) {
    ...
    // 4. Extract PRE tag (chords and lyrics layout)
    const preMatch = html.match(/<pre[^>]*>([\s\S]*?)<\/pre>/i);
    if (!preMatch) {
       throw new Error("Não foi possível localizar o bloco estrutural com a letra e os acordes nesta página do Cifra Club.");
    }

    const preHtml = preMatch[1];

    // Clean chords and extract lyrics, ignoring all tablatures/ASCII guitar finger grids line-by-line
    const rawLines = preHtml.split(/\r?\n/);
    const cleanedChordsLines: string[] = [];
    const lyricLines: string[] = [];

    // Walk through lines and create metadata for stateful checking
    ...
    for (let i = 0; i < linesMetadata.length; i++) {
      ...
      // Check if current line contains chords but actually belongs to a tablature (followed by a tab line)
      if (curr.hasChords) {
      ...
      // Clean chords formatting tags and clean chords representation according to Liloupro rules
      ...
      // Extract clean lyric line by removing all chord tags AND their inner chord content
      // e.g. "Tua beleza <b data-chord-name="G">G</b> me atrai" -> "Tua beleza me atrai"
      ...
      // Check if this line is a tab header or section header
      ...
      // Determine if this line actually contains text/lyrics
      // A valid lyric line should contain at least one word character with length >= 2 and not be a tab header, or be a section header
    ...
    // Render clean texts, suppressing consecutive empty lines that might have occurred from stripped blocks
    ...
    // Ask Gemini to fetch ONLY BPM and Time Signature (extremely lightweight, avoids 429 quota block completely)
    let bpm = 120;
      ...
      try {
        ...
        for (const modelName of GEMINI_FALLBACK_MODELS) {
          try {
            const response = await ai.models.generateContent({
              model: modelName,
              contents: `Forneça a estimativa de BPM (andamento, número inteiro) e compasso musical oficial para a canção "${extractedTitle}" do artista "${extractedArtist}".
Retorne estritamente em JSON:
{
  "bpm": número inteiro (ex: 78, 120, 130),
  "timeSignature": string (ex: "4/4", "3/4", "6/8")
        ...
        console.log("scrapeCifraClub: Note - metadados adicionais via Gemini indisponíveis (cota ou rede), usando defaults.");
      }
    }
